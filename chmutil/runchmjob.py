...
def main(arglist):
    ...
    desc = """
              ...
              if any tasks still need to be run. This is done by
              examining the <jobdir>/{run_dir}/<image dirs> directories and
              verifying image files exist for every tile listed in the
              <jobdir>/{chmconfig} config file. If not a new
              <jobdir>/{batchchm} configuration
              file is generated and a submit command is output to
              standard out that the user can invoke to submit those tasks.

              If no jobs need to be run then the message:
              'No chm jobs need to be run' will be output.

              NOTE: It is assumed no active jobs are running on this CHM job.
              ...
              """.format(version=chmutil.__version__,
                         ...
                         batchchm=CHMJobCreator.CONFIG_BATCHED_JOBS_FILE_NAME)
